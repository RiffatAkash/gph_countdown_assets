...
function checkIftarTime(todayDate, curTime) {
    ...
    if (diffTime<setTime && diffTime>=0){
        displayCounterIftar.innerHTML = printTimer(diffTime);
    }
    else {
        var lastTime = setTime+24*60*60;
        var sehriEnd = lastTime - curTime;
        displayCounterIftar.innerHTML = printTimer(sehriEnd);
    }
}
...
function printTimerSehri(sec) {
    ...
    var y = mm < 10? "0"+mm : mm;
    var z = ss < 10? "0"+ss : ss;
    return `<span class="hour_sehri absolute">${translteNum(x)}</span> <span class="minute_sehri absolute">${translteNum(y)}</span> <span class="second_sehri absolute">${translteNum(z)}</span>`;
}
